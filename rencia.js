...
let embed = new MessageEmbed().setColor("RANDOM").setAuthor(member.guild.name, member.guild.iconURL({dynamic: true})).setFooter("Kiezsa ❤️ Vahap" + member.guild.name).setTimestamp()
if(security === "şüpheli") {
 ...
 channel.send(`
\`>\` R A V E L İ O N 'a Hoşgeldin ${member}! Seninle birlikte ${member.guild.memberCount} kişiye ulaştık. <@&971006901875781633>
    
    \`>\` Hesabın \`${moment(member.user.createdAt).locale("tr").format("LLL")}\` tarihinde \`(${client.giris(member.user.createdAt)})\` açılmış. <a:kiezsaonay:971794831242297364  
...
\`>\` Sol taraftaki \<#971006902295224349>\ kanallarından birine girip \`isim-yaş\` belirterek kayıt olabilirsin.`)
